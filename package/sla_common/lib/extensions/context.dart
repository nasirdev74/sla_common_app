...
extension NullableContextExt on BuildContext? {
  static final _log = AppLog(BuildContext);

  bool get isValid {
    const tag = "NullableContextExtension";
    if (this != null) {
      if (this!.mounted) {
        return true;
      } else {
        _log.c(tag, "unmounted context", this);
      }
    } else {
      _log.c(tag, "context is null", this);
    }
    return false;
  }
}
...
extension ContextExt on BuildContext {
  ...
  double get height {
    if (kIsWeb && !kDebugMode) {
      return AppSettings.designSize.height.h;
    }
  ...
  double get width {
    if (kIsWeb && !kDebugMode) {
      return AppSettings.designSize.width.w;
    }
  ...
  ScrollBehavior get noScrollBar => ScrollConfiguration.of(this).copyWith(scrollbars: false);
